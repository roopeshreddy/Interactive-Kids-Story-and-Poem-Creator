...
def main():
    ...
    with col1:
        content_type = st.selectbox("What would you like to create?", ["Story", "Poem"])
    ...
    with col3:
        style_element = st.selectbox("Choose art style:", 
                                   ["Watercolor painting", "Digital art", "Cartoon", "Anime"])
        ...
        if theme:
            with st.spinner(f"Creating your magical {content_type.lower()}..."):
                ...
                with st.spinner("Crafting your illustration..."):
                    image_prompt = generator.generate_image_prompt(content, style_element, specific_element)
                    image_url = generator.generate_image(image_prompt)
                    st.image(image_url, caption=f"Your {content_type.lower()} illustration", use_container_width=True)
